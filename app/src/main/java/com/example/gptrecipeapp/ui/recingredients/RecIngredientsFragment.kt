...
class RecIngredientsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding.rvSeafoodList) {
            adapter = seafoodAdapter
            seafoodAdapter.submitList(getSeafoodList())
        }
        with(binding.rvVegetableList) {
            adapter = vegetableAdapter
            vegetableAdapter.submitList(getVegetableList())
        }
        with(binding.rvFruitList) {
            adapter = fruitAdapter
            fruitAdapter.submitList(getFruitList())
        }
        with(binding.rvProcessedList) {
            adapter = processedAdapter
            processedAdapter.submitList(getProcessedList())
        }
        with(binding.rvEtcList) {
            adapter = etcAdapter
            etcAdapter.submitList(getEtcList())
        }
    }
